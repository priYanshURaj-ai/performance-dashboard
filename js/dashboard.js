...
async function initDashboard() {
    try {
        ...
        setupEventListeners();
        renderDashboard();
        startAutoRefresh(); // Enable auto-refresh for real-time updates
    } catch (error) {
...
// ========================================
const JSONBIN_BIN_ID = '693bb6c643b1c97be9e88bb7';
...
// Check for data updates (polling from JSONBin.io)
...
function renderDashboard() {
    renderHeader();
    renderBandwidthOverview(); // Leadership view - Sprint bandwidth
    renderSummaryCards();
    renderMembersTable();
    renderCharts();
}
...
// Render Bandwidth Overview (Leadership View)
function renderBandwidthOverview() {
    ...
    // Calculate bandwidth utilization for each member
    const elapsedHours = currentSprint.elapsedHours || (currentSprint.elapsedWorkingDays * 8) || 80;
    ...
    const highUtilization = [...membersWithBandwidth]
        .filter(m => m.hoursLogged > 0) // Only members with logged hours
        .sort((a, b) => b.utilizationPercent - a.utilizationPercent)
        .slice(0, 3);

    // Sort by available hours (most available first) - members with low utilization
    ...
    // Render low utilization list
    if (lowUtilization.length === 0) {
        elements.lowUtilizationList.innerHTML = '<div class="no-bandwidth-data">No data available</div>';
    } else {
        elements.lowUtilizationList.innerHTML = lowUtilization.map((member, index) => `
            <div class="bandwidth-member" onclick="openMemberModal(${member.memberIndex})">
                ...
                <div class="bandwidth-member-stats">
                    ...
                    <span class="bandwidth-percent">${member.availableHours}h</span>
                    <span class="bandwidth-hours">free of ${member.elapsedHours}h</span>
                </div>
            </div>
        `).join('');
    }
...
function renderMembersTable() {
    ...
    const html = members.map(member => {
        ...
        // Helper to add 'zero' class when value is 0
...
function renderTopPerformersChart() {
    ...
    // Get top 5 performers by tasks done
...
// Fallback data - only structure, no mock values
// Real data comes from n8n workflow via JSONBin.io
function getSampleData() {
    console.warn('⚠️ Using fallback data structure - waiting for real JIRA data');
    ...
    return {
        ...
        sprints: null, // No mock sprint data - will be calculated by n8n
        periods: {
...
// Working hours per period (5 working days/week, 8 hours/day)
const DEFAULT_WORKING_HOURS = {
    '7days': 40,   // 5 days × 8 hours
    '15days': 88,  // ~11 working days
    '30days': 176  // ~22 working days
};
...
function populateModal(member) {
    ...
    // Reset sprint view to current
    currentSprintView = 'current';
    elements.sprintCurrentBtn?.classList.add('active');
    elements.sprintPreviousBtn?.classList.remove('active');
    
    // Populate sprint section
    ...
    // Bandwidth calculation for period
    ...
    // Top tickets for period
    renderTopTickets(metrics.topIssues || []);
}

// Populate sprint section with real JIRA data
function populateSprintSection(member) {
    ...
    // Check if sprint data is available from n8n workflow
    if (!dashboardData.sprints) {
        // Sprint data not yet loaded from n8n
        ...
        elements.sprintTicketsList.innerHTML = `
            <div class="no-tickets">
                <span>⏳</span> Run n8n workflow to load sprint data from JIRA
            </div>
        `;
        return;
    }
    ...
    // PROGRESSIVE BANDWIDTH for Current Sprint
    // Previous Sprint shows full bandwidth
    const isCurrentSprint = currentSprintView === 'current';
    
    if (isCurrentSprint) {
        // Current Sprint: Progressive bandwidth (Day 1 = 8h, Day 2 = 16h, etc.)
        ...
        // Utilization based on elapsed hours (not total sprint hours)
        const utilizationPercent = elapsedHours > 0 ? (hoursLogged / elapsedHours) * 100 : 0;
        const availableHours = Math.max(elapsedHours - hoursLogged, 0);
        const isOverutilized = hoursLogged > elapsedHours;
        
        // Update UI with progressive bandwidth
        ...
        elements.sprintHoursTotal.textContent = `of ${elapsedHours}h available (${sprintDay} days × 8h)`;
        ...
        // Previous Sprint: Full bandwidth (total hours)
        const totalHours = sprintInfo.totalHours || (sprintInfo.workingDays * 8) || 80;
        const hoursLogged = sprintMetrics?.hoursLogged || 0;
    ...
    // Sprint task summary from JIRA
    elements.sprintTotalTasks.textContent = sprintMetrics?.total || 0;
    elements.sprintDoneTasks.textContent = sprintMetrics?.done || 0;
...
// Render top tickets list
function renderTopTickets(topIssues) {
    if (!topIssues || topIssues.length === 0) {
        elements.topTicketsList.innerHTML = `
            <div class="no-tickets">
                <span>📭</span> No time logged on tickets in this period
            </div>
        `;
        return;
    }
...
// Render sprint tickets list from JIRA data
function renderSprintTickets(sprintIssues) {
    if (!sprintIssues || sprintIssues.length === 0) {
        elements.sprintTicketsList.innerHTML = `
            <div class="no-tickets">
                <span>📭</span> No tasks assigned in this sprint period
            </div>
        `;
        return;
    }
    ...
    // Sort by hours logged (descending) - real JIRA time data
